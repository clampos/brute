...
exports.ProgressiveOverloadService = void 0;
...
class ProgressiveOverloadService {
     ...
     */
    static generateRPETable(programLength) {
        const rpeTable = [];
     ...
     */
    static async getPerformanceHistory(userId, exerciseId, weeks = 3) {
        const workouts = await prisma_js_1.prisma.workout.findMany({
            where: {
                userProgram: {
                    userId,
                    status: "ACTIVE",
    ...
    static async getLastWorkoutSetsForExercise(userId, exerciseId) {
        const workout = await prisma_js_1.prisma.workout.findFirst({
            where: {
                userProgram: {
                    userId,
                    status: "ACTIVE",
    ...
    static calculateSetRecommendation(previousSet, currentRPE, sets, progressionType) {
        ...
        if (progressionType === "MANUAL_JUMP") {
            ...
            recommendedReps = Math.max(1, Math.floor((previousSet.targetReps || previousSet.reps) * 0.9));
        }
        else if (progressionType === "OVERPERFORMANCE") {
            recommendedWeight = previousSet.weight * (1 + this.WEIGHT_INCREASE_PERCENT);
            recommendedReps = this.calculateVolumePreservingReps(previousSet.weight, previousSet.reps, recommendedWeight, sets);
        }
            ...
            recommendedWeight = previousSet.weight;
            recommendedReps = Math.min((previousSet.targetReps || previousSet.reps) + 1, this.REP_CAP);
        }
    ...
    static buildSetRecommendations(previousSets, setCount, currentRPE, progressionType) {
        ...
        for (let index = 0; index < setCount; index += 1) {
            normalizedSets[index] =
                previousSets[index] || previousSets[previousSets.length - 1];
        }
        return normalizedSets.map((previousSet, idx) => {
            const setRecommendation = this.calculateSetRecommendation(previousSet, currentRPE, setCount, progressionType);
            return {
                ...setRecommendation,
                setNumber: idx + 1,
            };
     ...
     * Clamp between 8-12
     */
    static calculateVolumePreservingReps(previousWeight, previousReps, newWeight, sets) {
     ...
     */
    static async getUserProgramDetails(userId) {
        const userProgram = await prisma_js_1.prisma.userProgram.findFirst({
            where: {
                userId,
                status: "ACTIVE",
            },
     ...
     */
    static async getProgressionRecommendations(userId, exerciseIds, setCounts = []) {
        ...
        try {
            ...
            for (let index = 0; index < exerciseIds.length; index += 1) {
                ...
                if (history.length === 0 || lastWorkoutSets.length === 0) {
                    const defaultSetRecommendations = Array.from({ length: setCount }, (_, idx) => ({
                        setNumber: idx + 1,
                ...
                // If user performs below target reps for 2 consecutive weeks
                    ...
                    const underperformed = last2Weeks.every((week) => week.targetReps > 0 && week.actualReps < week.targetReps);
                    if (underperformed) {
                        // Reduce weight by 10%
                        recommendedWeight =
                    ...
                    // If user performs >10% above target reps for 2 consecutive weeks
                        ...
                        const overperformed = last2Weeks.every((week) => week.targetReps > 0 &&
                            week.actualReps >
                                week.targetReps * this.OVERPERFORMANCE_THRESHOLD);
                        if (overperformed) {
                            recommendedWeight =
                                currentWeight * (1 + this.WEIGHT_INCREASE_PERCENT);
                            recommendedReps = this.calculateVolumePreservingReps(currentWeight, lastReps, recommendedWeight, setCount);
                ...
                const setRecommendations = this.buildSetRecommendations(lastWorkoutSets, setCount, currentRPE, progressionType);
     ...
     */
    static async saveWorkoutAndCalculateProgression(userId, workoutData) {
        try {
            ...
            const workout = await prisma_js_1.prisma.workout.create({
                data: {
            ...
            for (const exercise of workoutData) {
                const workoutExercise = await prisma_js_1.prisma.workoutExercise.create({
                    data: {
                ...
                for (let i = 0; i < exercise.sets.length; i++) {
                    ...
                    await prisma_js_1.prisma.workoutSet.create({
                        data: {
            ...
            await prisma_js_1.prisma.userProgram.update({
            ...
            let recommendations = [];
            if (isEndOfWeek) {
                recommendations = await this.getProgressionRecommendations(userId, exerciseIds);
            }
            else {
    ...
    static async getWeeklySummary(userId, currentWeek, exerciseIds) {
        try {
            ...
            const workoutsPastThreeWeeks = await prisma_js_1.prisma.workout.findMany({
                where: {
                    userProgram: {
                        userId,
                        status: "ACTIVE",
            ...
            if (weeklyData[currentWeek]) {
                for (const exId of exerciseIds) {
                    ...
                    if (currentMaxReps > previousMaxReps) {
                        const exercise = await prisma_js_1.prisma.exercise.findUnique({
                            where: { id: exId },
                        });
...
exports.ProgressiveOverloadService = ProgressiveOverloadService;
// Constants from your rules
ProgressiveOverloadService.REP_CAP = 15;
ProgressiveOverloadService.REP_FAIL = 5;
ProgressiveOverloadService.WEIGHT_INCREASE_PERCENT = 0.05;
ProgressiveOverloadService.WEIGHT_DECREASE_PERCENT = 0.1;
ProgressiveOverloadService.MANUAL_INCREASE_THRESHOLD = 1.1;
ProgressiveOverloadService.OVERPERFORMANCE_THRESHOLD = 1.1;
