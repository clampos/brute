    ...
    try {
      ...
      if (existingActiveProgram) {
        return res
          ...
          .json({
            error:
              "You already have an active programme. Please complete or cancel it first.",
          });
      }
...
router.get(
  ...
  async (req: Request, res: Response): Promise<any> => {
    try {
      ...
      const progressionData = await calculateProgressionWeights(
        userProgram.id,
...
async function calculateProgressionWeights(
